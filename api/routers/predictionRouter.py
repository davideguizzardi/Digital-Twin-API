...
def getPredictionRouter():
    ...
    def Get_Power_Consumption_Prediction():
        model=keras.saving.load_model(f"{FILENAME_POWER}.keras", custom_objects=None, compile=False, safe_mode=True)
        model.compile(optimizer="adam",loss="mse",metrics=["mse"])

        scaler=None
        if os.path.exists(f"{FILENAME_POWER}.bin"):
            scaler=scalerload(f"{FILENAME_POWER}.bin")
        selected_date=datetime.now().astimezone(tz.tzlocal())
        query = (
    ...
    def Get_Power_Consumption_Prediction_For_Service(device_id,service):
        ...
        if device_id=="73f90a95e5dba7d52e5ad10517b1dec6":
            return Get_Power_Consumption_Prediction_For_WashingMachine(service)
        model=keras.saving.load_model(f"{FILENAME_POWER}.keras", custom_objects=None, compile=False, safe_mode=True)
        model.summary()
        model.compile(optimizer="adam",loss="mse",metrics=["mse"])
    ...
    @prediction_router.get("/washing/{service}")#TODO: this is a copy of the previous one, pls refactor
    def Get_Power_Consumption_Prediction_For_WashingMachine(service):
        device_id="73f90a95e5dba7d52e5ad10517b1dec6"
        filename="./prediction_models/washing_power_18-03-2025-14-08_Prev120_Next60_noscalerscaling"
        model=keras.saving.load_model(f"{filename}.keras", custom_objects=None, compile=False, safe_mode=True)
        model.summary()
        model.compile(optimizer="adam",loss="mse",metrics=["mse"])

        scaler=None
        if os.path.exists(f"{filename}.bin"):
            scaler=scalerload(f"{filename}.bin")
        selected_date=datetime.now().astimezone(tz.tzlocal())
        query = (
            "SELECT date, timestamp, power FROM ("
            "    SELECT strftime('%d-%m-%Y %H:%M', timestamp, 'unixepoch', 'localtime') as 'date',"
            "           timestamp, SUM(power) AS 'power'"
            "    FROM Device_History"
            f"    WHERE timestamp<={selected_date.timestamp()} and device_id=\"{device_id}\""
            "    GROUP BY timestamp"
            "    ORDER BY timestamp DESC"
            f"    LIMIT {PREVIOUS_POWER_STEPS}"#TODO:extract this value from model
            ") AS recent_rows"
        ...
        dataset_complete["state"] = np.where(dataset_complete["power"] > 5, 1, 0)
        dataset_complete["duration"] = 0
        counter = 0

        # Iterate through rows to calculate duration
        for i in range(1, len(dataset_complete)):
            if dataset_complete.loc[i, "state"] == 1:
                if dataset_complete.loc[i - 1, "state"] == 0:
                    counter = 1  # Reset on transition
                else:
                    counter += 1  # Increment while "on"
            else:
                counter = 0  # Reset when "off"
            
            dataset_complete.loc[i, "duration"] = counter
        ...
        last_date=last_date+timedelta(minutes=1)
        last_power=float(consumption_data[-1]["power"])
        ...
        for i in range(5):
            ...
            new_row = {
                ...
                "duration":i+1 if state=="on" else 0,
            }
            result.append({"date":last_date.strftime('%d-%m-%Y %H:%M'),"power":last_power,"power_unit":unit})
            dataset_complete = pd.concat([dataset_complete, pd.DataFrame([new_row])], ignore_index=True)
            last_date=last_date+timedelta(minutes=1)
        ...
        for i in range(FUTURE_POWER_STEPS):
            date_str=f"{last_date.strftime('%d-%m-%Y %H:%M')}"
            energy_cons=float(y_predict[i])
            result.append({"date":date_str,"power":energy_cons,"power_unit":unit})
            last_date+=timedelta(minutes=1)

        
        base = Get_Power_Consumption_Prediction()

        for i,item in enumerate(base):
            result[i]["power"]=float(result[i]["power"])+float(item["power"])
        ...
        if device_id.split(".")[0]=="virtual":
            ...
            if device_info["status_code"]==200:
                ...
                if state!=current_state and state not in ["same","on|off"]:
                    ...
                    time_delta=min(60-selected_date.minute,state_duration)
                    consumption_delta.append(power_delta*(time_delta/60)) #need to convert it into Wh
                    state_duration=state_duration-time_delta

                    while state_duration>0:#i need to add also for the second hour
                        time_delta=min(60,state_duration)
                        consumption_delta.append((power_delta*time_delta)/60)
                        state_duration=state_duration-time_delta
